...
app.controller("preliminaryListCont",["$scope","$http","sessionService","$stateParams","$state","$interval","toastr",function($scope,$http,sessionService,$stateParams,$state,$interval,toastr){
    $scope.preliminarylist = {
        clientName: "",
        clientId: "",
        clientPhone: "",
        orderNumber: "",
    ...
    //                 $scope.preliminarylist.area=data.text;
    //                 $scope.preliminarylist.orgCode=data.code;
    //                 $scope.showAreaTreeview=false;
    //                 $scope.$apply();
    //             }
    //         };
    ...
    //     $scope.preliminarylist.area=text;
    //     $scope.preliminarylist.orgCode=code;
    //     $scope.showAreaTreeviewResult=false;
    //
    // }
    ...
    function list(page) {
        ...
        $.ajax({
            type: 'GET',
            async:false,
            url: "/firsttrial/getFirstTrialList?orgCode="+$scope.preliminarylist.orgCode+"&userName="+$scope.preliminarylist.clientName+"&userNationalid="+$scope.preliminarylist.clientId+"&userPhone="+$scope.preliminarylist.clientPhone+"&orderCode="+$scope.preliminarylist.orderNumber+"&page="+page+"&size="+$scope.preliminarylist.pageSize,
            success:function (res) {
                console.log(res);
                if(res.success&&res.executed)
                {
                    $scope.none=false;
                    $scope.rowsList=res.firstTrialListInfoS;
                    $scope.totalData=res.total;
                    $scope.PageCount = Math.ceil($scope.totalData / $scope.preliminarylist.pageSize)==0?1:Math.ceil($scope.totalData / $scope.preliminarylist.pageSize);
                ...
                $scope.option = {
                    ...
                    size:$scope.preliminarylist.pageSize,
                    ...
                    changeSize:function (page,size) {
                        $scope.preliminarylist.pageSize=size;
                        list(page);
                    }
                };
            }, error:function (res) {
    ...
    $scope.reset=function(){
        ...
        $scope.preliminarylist = {
            ...
            orgCode:"",
            creditCode:"",
            pageSize:$scope.preliminarylist.pageSize
        }
        list(1);
    };
    //新窗口打开页面
    $scope.openNewPage=function (bizid,type,userPhone,userCode,state,orderCode,page,index,username) {
        //下一个页面要用的数据
        $scope.auditParams={
            bizid:bizid,
            type:type,
        ...
        sessionService.setObject("preliminaryParams",$scope.auditParams);
        window.open($state.href('preliminaryDetails'),'_blank');
    };

    $scope.ischeckXinshen=function(bizid,type,userPhone,userCode,state,orderCode,page,index,isAuth,username) {
        $http({
            method:"PUT",
            url:"/firsttrial/"+bizid+"/updateFirstTrialByBid",
        }).then(function (res) {
            console.log(res);
